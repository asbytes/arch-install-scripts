#compdef pacstrap-noroot genfstab arch-chroot

_pacstrap-noroot_args=(
    '-h[display help]'
)

_pacstrap-noroot_args_nonh=(
...
_pacstrap-noroot(){
    if [[ -z ${(M)words:#--help} && -z ${(M)words:#-h} ]]; then
        case $words[CURRENT] in
            -c*|-d*|-i*)
                _arguments -s "$_pacstrap-noroot_args_nonh[@]"
                ;;
            -*)
                _arguments -s : \
                    "$_pacstrap-noroot_args[@]" \
